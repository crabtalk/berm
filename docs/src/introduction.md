...
**[berm](./berm/overview.md)** is a sandbox for harnesses: it pins a guest by
hash, compiles it once, instantiates it per invocation, and hands it only what
it was given. `bermd` deploys harnesses and serves their tools over MCP.
...
that makes a guest a *harness* lives on the berm side, which is what leaves
