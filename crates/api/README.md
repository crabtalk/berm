...
dispatch into a syscall its author did not mean.

`Program`, `ToolSpec` and `Output` are the control API's own shapes.
`Output::Failed` is the program's own report and arrives with the same `200` a
