...
//!
//! A source names a harness, a tool and an argument blob, and berm runs a
...
impl Service {
    ...
    ///
    /// Infallible by design. A source outlives the harness it points at, so a
    /// target undeployed since is one of the things that happens here.
    pub(crate) async fn dispatch(&self, harness: &str, tool: &str, args: Vec<u8>) {
        match self.call(harness, tool, args).await {
            Ok(Ok(result)) => {
                tracing::debug!(harness, tool, bytes = result.len(), "dispatched")
            }
            Ok(Err(failure)) => tracing::warn!(harness, tool, "{failure}"),
            Err(error) => tracing::error!(harness, tool, "{error:#}"),
        }
    }
}
