//! `berm run` — run one of a harness's tools.
...
/// A harness that ran and said no, which is not berm failing to run it — the
/// two leave by different doors so a script can tell them apart.
const FAILED: u8 = 2;

pub fn run(client: &Client, harness: &str, tool: &str, inline: Option<&str>) -> Result<ExitCode> {
    match client.run(harness, tool, arguments(inline)?)? {
        Output::Done(result) => {
            println!("{result}");
            Ok(ExitCode::SUCCESS)
        }
        Output::Failed(failure) => {
            eprintln!("{harness}.{tool} failed: {failure}");
            Ok(ExitCode::from(FAILED))
        }
    }
}

/// What was typed, what was piped in, or nothing — which is what a harness
...
/// object: `echo '{}' | berm run` has to reach the harness as the same bytes
